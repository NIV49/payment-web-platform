...
用户和角色查询表单只在显式查询或重置时提交；部门树选择是独立的即时筛选，并只发送标量 `deptId`。角色表单的 `menuIds` 只管理导航节点；独立“功能权限”抽屉通过 `/api/v1/iam/roles/{roleId}/grants` 管理精确的 RoleGrant。抽屉明确展示复合操作的依赖权限，勾选动作时补齐依赖，取消依赖时移除已不可用的动作，并拒绝保存历史遗留的缺依赖组合。system/non-assignable 角色不可变更，包含当前页面无法无损表达的 Grant 时抽屉只读。
