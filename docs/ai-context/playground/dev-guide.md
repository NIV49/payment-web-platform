...
- `demos/examples` 源码保留作开发参考，但不注册为产品路由，也不要求产品后端实现其示例接口。
- 启用后端动态菜单，并以显式本地路由白名单组成 `mixed` 访问模式。
- refresh token 是后续目标能力，且只能使用独立 HttpOnly Cookie；完整安全协议批准前继续关闭。
- 用户管理需要角色分配 UI；当前产品已经使用 `roleIds` 多选和后端委派校验实现。
