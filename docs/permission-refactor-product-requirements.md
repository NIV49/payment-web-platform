...
账号域与会话边界已由 [ADR-0008](adr/0008-isolate-three-backoffice-account-domains-and-sessions.md) 定版：三后台使用独立应用账号和会话，同域多 Membership 仍允许，工作区仅由可信服务端上下文解析。以下其他内容尚未完全定案，需要在技术方案前由产品与技术共同确认：

1. 临时密码具体有效时长和失败锁定策略；
2. 代理关系解除申请的有效期和超时策略；
3. 是否启用平台风险强制解除，以及具体授权角色；
4. 代理商有效关系期间可见的订单字段清单；
5. 历史订单脱敏字段的最终清单；
6. 解除后是否开放历史订单导出，开放时的数量限制、水印和审批要求；
7. 代理商、销售和商户佣金的具体计算、调整与结算规则；
8. 直连商户转间连、间连商户转直连和更换代理商的审批流程；
9. 数据范围是否还需要市场、国家、渠道和资金账户维度；
10. 平台、代理商和商户各自的预置角色清单；
11. 权限缓存失效和 Session 撤销的目标时效；
12. 历史订单关系快照无法补齐时的迁移策略；
13. 新旧权限影子比对周期和灰度范围。
