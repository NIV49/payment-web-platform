...
Every non-current `initialStateHistory` key requires exact retained `queueHistoryEvidence`: the immutable historical target and recomputed Rulebook/Judge manifests, the matching Queue state, and two trusted independent signed PASS implementation reviews bound to that key, target tree, Queue digest, and successful Judge executions. Retained targets must be a strict ancestor chain; merge transitions require a later single-parent newly evaluated reconciliation. The field may be absent only for a one-state transcript whose sole key is current. Existing arbitrary 64-hex bootstrap placeholders are invalid and receive no allowlist or migration waiver.

Non-review sources are evidence-bound too. A `judge` Queue `checkId` must resolve in the evaluated Judge registry and have a matching execution in a Review Result that is otherwise fully valid and signs the current `queueDigest`. A `build` or `test` Queue `checkId` must not alias a Judge registry entry and must have a matching command execution in such a valid signed review. The v2 schema has no separate typed build/test gate registry, so free-text criteria, verification instructions, or unsigned CI labels are not gate definitions. The same non-review `checkId` may identify only one Queue source; independent reviewers may attest the same execution ID, but their commands must agree.
