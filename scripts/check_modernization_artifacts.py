...
BUNDLE_OPTIONAL_FIELDS = {"queueHistoryEvidence"}
QUEUE_HISTORY_EVIDENCE_FIELDS = {
    "evaluatedSnapshot",
    "queueItems",
    "reviewResults",
}
...
def _canonical_json_equal(left: Any, right: Any) -> bool:
    """Compare strict JSON values without Python's bool/int/float aliases."""
...
def validate_policy(policy: Any) -> list[str]:
    ...
    errors: list[str] = []
    if type(policy["schemaVersion"]) is not int or policy["schemaVersion"] != 2:
        errors.append(
            "payment modernization policy schemaVersion must be the integer 2"
        )
...
def load_judge_registry(
    ...
    if (
        ...
        return {}, ["Judge registry schemaVersion must be the integer 1"]
    if not isinstance(payload.get("checks"), list):
        return {}, ["Judge registry checks must be a list"]
    errors: list[str] = []
...
def validate_review_result(
    ...
    *,
    ...
    expected_external_execution_check_ids: set[str] | None = None,
) -> list[str]:
    ...
    if isinstance(review["commandsRun"], list):
        for index, execution in enumerate(review["commandsRun"]):
            ...
            if isinstance(check_id, str):
                ...
                if (
                    expected_judge_registry is not None
                    and registered is None
                    and (
                        expected_external_execution_check_ids is None
                        or check_id not in expected_external_execution_check_ids
                    )
                ):
                    errors.append(
                        "Review Result commandsRun checkId is neither in the Judge "
                        "registry nor referenced by a build/test Queue source"
                    )
...
def _validate_queue_state_transition(
    ...
    if (
        ...
        and previous.get("evaluatedVersionKey")
        == current.get("evaluatedVersionKey")
    ):
        errors.append(
    ...
    if (
        previous_status == current_status
        and previous.get("resolution") != current.get("resolution")
    ):
        errors.append(
            f"Queue Item {fingerprint} resolution cannot change without a status transition"
        )
    previous_rounds = previous.get("failedReviewRounds")
    current_rounds = current.get("failedReviewRounds")
...
def validate_queue_initial_state_history(
    history: Any,
    fingerprint: str,
    *,
    current_evaluated_version_key: str | None = None,
    history_evidence_is_validated_by_bundle: bool = False,
) -> tuple[list[str], Mapping[str, Any] | None]:
    if not isinstance(history, list) or not history:
        return [
            f"Queue Item {fingerprint} initialStateHistory must be a non-empty list"
        ], None
    errors: list[str] = []
    valid_states: list[Mapping[str, Any]] = []
    version_key_indexes: dict[str, int] = {}
    for index, state in enumerate(history):
        if not isinstance(state, Mapping) or set(state) != QUEUE_STATE_FIELDS:
            errors.append(
                f"Queue Item {fingerprint} initialStateHistory[{index}] "
                "must use the exact state schema"
            )
            continue
        status = state.get("status")
        if status not in QUEUE_TRANSITIONS:
            errors.append(
                f"Queue Item {fingerprint} initialStateHistory[{index}] "
                "status is invalid"
            )
        evaluated_version_key = state.get("evaluatedVersionKey")
        if (
            not isinstance(evaluated_version_key, str)
            or DIGEST_PATTERN.fullmatch(evaluated_version_key) is None
        ):
            errors.append(
                f"Queue Item {fingerprint} initialStateHistory[{index}] "
                "evaluatedVersionKey must be a sha256 key"
            )
        elif evaluated_version_key in version_key_indexes:
            errors.append(
                f"Queue Item {fingerprint} initialStateHistory[{index}] "
                "evaluatedVersionKey is reused from "
                f"initialStateHistory[{version_key_indexes[evaluated_version_key]}]"
            )
        else:
            version_key_indexes[evaluated_version_key] = index
        failed_review_rounds = state.get("failedReviewRounds")
        if (
            type(failed_review_rounds) is not int
            or not 0 <= failed_review_rounds <= 3
        ):
            errors.append(
                f"Queue Item {fingerprint} initialStateHistory[{index}] "
                "failedReviewRounds must be an integer from 0 to 3"
            )
        valid_states.append(state)
    if len(valid_states) != len(history):
        return errors, valid_states[-1] if valid_states else None
    first = valid_states[0]
    if first.get("status") != "open":
        errors.append(
            f"Queue Item {fingerprint} initialStateHistory must start open"
        )
    if first.get("failedReviewRounds") != 0:
        errors.append(
            f"Queue Item {fingerprint} initialStateHistory must start with "
            "zero failed review rounds"
        )
    for previous, current in zip(valid_states, valid_states[1:]):
        if previous.get("status") == current.get("status"):
            errors.append(
                f"Queue Item {fingerprint} initialStateHistory must contain "
                "only real status transitions"
            )
        errors.extend(
            _validate_queue_state_transition(previous, current, fingerprint)
        )
    has_non_current_state = (
        len(valid_states) > 1
        if current_evaluated_version_key is None
        else any(
            state.get("evaluatedVersionKey") != current_evaluated_version_key
            for state in valid_states
        )
    )
    if has_non_current_state and not history_evidence_is_validated_by_bundle:
        errors.append(
            f"Queue Item {fingerprint} non-current initialStateHistory entries "
            "require bundle-level retained signed history evidence validation"
        )
    return errors, valid_states[-1]


def validate_queue_origin_execution(
    execution: Any,
    check_id: str,
) -> list[str]:
    if (
        not isinstance(execution, Mapping)
        or set(execution) != COMMAND_EXECUTION_FIELDS
    ):
        return [
            "Build/test failureSource.originExecution must use the exact "
            "execution schema"
        ]
    errors: list[str] = []
    if execution.get("checkId") != check_id:
        errors.append(
            "Build/test failureSource.originExecution checkId must match "
            "failureSource.checkId"
        )
    if (
        not isinstance(execution.get("command"), str)
        or not execution["command"].strip()
    ):
        errors.append(
            "Build/test failureSource.originExecution command must not be empty"
        )
    if (
        not isinstance(execution.get("targetCommitSha"), str)
        or FULL_SHA_PATTERN.fullmatch(execution["targetCommitSha"]) is None
    ):
        errors.append(
            "Build/test failureSource.originExecution targetCommitSha must be "
            "a lowercase full SHA"
        )
    if type(execution.get("exitCode")) is not int or execution["exitCode"] == 0:
        errors.append(
            "Build/test failureSource.originExecution exitCode must be a "
            "non-zero integer"
        )
    if (
        not isinstance(execution.get("resultDigest"), str)
        or DIGEST_PATTERN.fullmatch(execution["resultDigest"]) is None
    ):
        errors.append(
            "Build/test failureSource.originExecution resultDigest must be "
            "a sha256 digest"
        )
    return errors
...
def validate_queue_item(
    ...
    *,
    ...
    if set(item) != QUEUE_ITEM_FIELDS:
        errors.append("Queue Item must use the exact schema")
    if (
        type(item.get("queueItemSchemaVersion")) is not int
        or item["queueItemSchemaVersion"] != QUEUE_ITEM_SCHEMA_VERSION
    ):
        errors.append("Queue Item queueItemSchemaVersion must be the integer 2")
    ...
    expected_resolutions = {
        ...
        "implementing": {"unresolved"},
        "reviewing": {"unresolved"},
        "closed": {"fixed", "flaky", "rejected"},
        "human-decision": {"deferred"},
    }
    if resolution not in {"unresolved", "fixed", "flaky", "rejected", "deferred"}:
        errors.append("Queue Item resolution is invalid")
    elif (
        item.get("status") in expected_resolutions
        and resolution not in expected_resolutions[item["status"]]
    ):
        errors.append("Queue Item status and resolution conflict")
    if (
    ...
    for field in ("evidence", "dependencies"):
        try:
            _canonical_json_bytes(item.get(field))
        except ContractError:
            errors.append(f"Queue Item {field} must contain canonical JSON values")
    fingerprint = (
        item["fingerprint"]
        if isinstance(item.get("fingerprint"), str)
        else "<invalid-fingerprint>"
    )
    history_errors, initial_final_state = validate_queue_initial_state_history(
        item.get("initialStateHistory"),
        fingerprint,
        current_evaluated_version_key=item.get("evaluatedVersionKey")
        if isinstance(item.get("evaluatedVersionKey"), str)
        else None,
        history_evidence_is_validated_by_bundle=(
            history_evidence_is_validated_by_bundle
        ),
    )
    errors.extend(history_errors)
    if (
        initial_final_state is not None
        and initial_final_state.get("evaluatedVersionKey")
        == item.get("evaluatedVersionKey")
        and any(
            item.get(field) != initial_final_state.get(field)
            for field in QUEUE_STATE_FIELDS
        )
    ):
        errors.append(
            "Queue Item current state conflicts with its initialStateHistory"
        )
    ...
    elif resolution == "flaky" and source_type not in {"build", "test"}:
        errors.append("Queue Item flaky resolution is limited to build/test sources")
    ...
    elif source_type in {"build", "test"}:
        if set(source) != {"type", "checkId", "originExecution"}:
            errors.append(
                f"{source_type} failureSource must contain type, checkId, "
                "and originExecution"
            )
            ...
            errors.extend(
                validate_queue_origin_execution(
                    source["originExecution"],
                    source["checkId"],
                )
            )
    else:
...
def validate_non_review_queue_traceability(
    reviews: Sequence[Mapping[str, Any]],
    queue_items: Sequence[Any],
    judge_registry: Mapping[str, Mapping[str, Any]],
    registered_rule_ids: set[str],
    slice_rule_ids: set[str],
    repository: Path,
    target_commit: str,
) -> list[str]:
    errors: list[str] = []
    executions_by_check_id: dict[str, list[Mapping[str, Any]]] = {}
    for review in reviews:
        executions = review.get("commandsRun")
        ...
        for execution in executions:
            ...
            check_id = execution.get("checkId")
            if isinstance(check_id, str):
                executions_by_check_id.setdefault(check_id, []).append(execution)
    ...
    for item in queue_items:
        ...
        if source_type == "rule":
            ...
            if rule_id not in registered_rule_ids:
                errors.append(
                    "Rule Queue source is not in the evaluated Rule Card registry: "
                    f"{rule_id}"
                )
            elif rule_id not in slice_rule_ids:
                errors.append(
                    "Rule Queue source is not part of the current Capability Slice: "
                    f"{rule_id}"
                )
            continue
        check_id = source.get("checkId")
        if source_type not in {"judge", "build", "test"} or not isinstance(
            check_id, str
        ):
            continue
        source_counts[check_id] = source_counts.get(check_id, 0) + 1
        executions = executions_by_check_id.get(check_id, [])
        registered = judge_registry.get(check_id)
        if source_type == "judge":
            if registered is None:
                errors.append(
                    f"Judge Queue source is not in the evaluated registry: {check_id}"
                )
            elif not executions:
                errors.append(
                    "Judge Queue source lacks trusted signed execution evidence: "
                    f"{check_id}"
                )
        else:
            label = source_type.capitalize()
            if registered is not None:
                errors.append(
                    f"{label} Queue source must not reuse an evaluated Judge "
                    f"registry checkId: {check_id}"
                )
            if not executions:
                errors.append(
                    f"{label} Queue source lacks trusted signed execution evidence: "
                    f"{check_id}"
                )
            origin_execution = source.get("originExecution")
            if isinstance(origin_execution, Mapping):
                origin_target = origin_execution.get("targetCommitSha")
                if isinstance(origin_target, str):
                    strict_closure = item.get("status") == "closed"
                    ancestry_valid = not (
                        strict_closure and origin_target == target_commit
                    )
                    if ancestry_valid:
                        try:
                    ...
                    if not ancestry_valid:
                        relationship = (
                            "a strict ancestor of"
                            if strict_closure
                            else "an ancestor of"
                        )
                        errors.append(
                            f"{label} Queue source originExecution target is not "
                            f"{relationship} the evaluated snapshot: {check_id}"
                        )
                origin_command = origin_execution.get("command")
                if isinstance(origin_command, str) and any(
                    execution.get("command") != origin_command
                    for execution in executions
                ):
                    errors.append(
                        f"{label} Queue source trusted execution does not match "
                        f"the immutable originExecution command: {check_id}"
                    )
        commands = {
            execution.get("command")
            for execution in executions
            if isinstance(execution.get("command"), str)
        }
        if len(commands) > 1:
            errors.append(
                f"Queue source {check_id} trusted executions disagree on command"
            )

    for check_id, count in source_counts.items():
        if count != 1:
            errors.append(
                "checkId must identify exactly one non-review Queue source: "
                f"{check_id}"
            )
    return errors


def _queue_immutable_root(item: Mapping[str, Any]) -> dict[str, Any]:
    return {
        key: value
...
def validate_queue_history_evidence(
    raw_evidence: Any,
    *,
    queue_items: Sequence[Any],
    capability: Mapping[str, Any],
    ...
    repository: Path,
    baseline_policy: Mapping[str, Any],
    trusted_reviewers: Mapping[str, Mapping[str, Any]],
    current_review_ids: set[str],
    current_review_keys: set[str],
) -> list[str]:
    """Validate self-contained signed evidence for non-current bootstrap states."""

    if not isinstance(raw_evidence, list):
        return ["queueHistoryEvidence must be a list"]
    ...
    required_states: dict[tuple[str, str], Mapping[str, Any]] = {}
    for fingerprint, item in current_items.items():
        history = item.get("initialStateHistory")
        if not isinstance(history, list):
            continue
        for state in history:
            if not isinstance(state, Mapping):
                continue
            history_key = state.get("evaluatedVersionKey")
            if (
                isinstance(history_key, str)
                and history_key != item.get("evaluatedVersionKey")
            ):
                required_states[(fingerprint, history_key)] = state

    evidence_targets: dict[tuple[str, str], str] = {}
    evidence_items: dict[tuple[str, str], Mapping[str, Any]] = {}
    seen_snapshot_keys: set[str] = set()
    seen_review_ids = set(current_review_ids)
    seen_review_keys = set(current_review_keys)

    for evidence_index, evidence_entry in enumerate(raw_evidence):
        prefix = f"queueHistoryEvidence[{evidence_index}]"
        if (
            not isinstance(evidence_entry, Mapping)
            or set(evidence_entry) != QUEUE_HISTORY_EVIDENCE_FIELDS
        ):
            errors.append(f"{prefix} must use the exact history evidence schema")
            continue
        snapshot = evidence_entry["evaluatedSnapshot"]
        evidence_queue = evidence_entry["queueItems"]
        evidence_reviews = evidence_entry["reviewResults"]
        if (
            not isinstance(snapshot, Mapping)
            or set(snapshot) != EVALUATED_SNAPSHOT_FIELDS
        ):
            errors.append(f"{prefix}.evaluatedSnapshot must use the exact v2 schema")
            continue
        history_key = snapshot.get("evaluatedVersionKey")
        target_commit = snapshot.get("targetCommitSha")
        if (
            not isinstance(history_key, str)
            or DIGEST_PATTERN.fullmatch(history_key) is None
        ):
            errors.append(
                f"{prefix}.evaluatedSnapshot evaluatedVersionKey must be a sha256 key"
            )
        elif history_key in seen_snapshot_keys:
            errors.append(
                f"{prefix}.evaluatedSnapshot evaluatedVersionKey must be unique"
            )
        else:
            seen_snapshot_keys.add(history_key)
        if (
            not isinstance(target_commit, str)
            or FULL_SHA_PATTERN.fullmatch(target_commit) is None
        ):
            errors.append(
                f"{prefix}.evaluatedSnapshot targetCommitSha must be a lowercase full SHA"
            )
            target_commit = None

        history_policy: Mapping[str, Any] | None = None
        rule_manifest_errors: list[str] = []
        judge_manifest_errors: list[str] = []
        if target_commit is not None:
            try:
                ...
                if target_commit == current_snapshot.get("targetCommitSha"):
                    raise EvidenceError(
                        "history evidence target must precede the current target"
                    )
                validate_git_ancestor(
                    repository,
                    target_commit,
                    current_snapshot["targetCommitSha"],
                )
                history_policy = load_policy(repository, target_commit)
            except (EvidenceError, ContractError, KeyError, TypeError) as error:
                errors.append(
                    f"{prefix}.evaluatedSnapshot is not an immutable historical "
                    f"ancestor: {error}"
                )
            if history_policy is not None:
                for field in (
                    ...
                    if history_policy[field] != baseline_policy[field]:
                        errors.append(
                            f"{prefix} historical policy {field} does not match "
                            "the trusted baseline"
                        )
                for field in ("rulebookPaths", "ruleCardPaths", "judgePaths"):
                    if not set(baseline_policy[field]).issubset(
                        history_policy[field]
                    ):
                        errors.append(
                            f"{prefix} historical policy removed baseline {field}"
                        )
                _, rule_manifest_errors = _resolve_content_manifest(
                    repository,
                    target_commit,
                    snapshot.get("rulebookManifest"),
                    history_policy["rulebookPaths"],
                    manifest_name=f"{prefix}.evaluatedSnapshot rulebookManifest",
                    digest_field="rulebookDigest",
                )
                _, judge_manifest_errors = _resolve_content_manifest(
                    repository,
                    target_commit,
                    snapshot.get("judgeManifest"),
                    history_policy["judgePaths"],
                    manifest_name=f"{prefix}.evaluatedSnapshot judgeManifest",
                    digest_field="judgeDigest",
                )
                errors.extend(rule_manifest_errors)
                errors.extend(judge_manifest_errors)
                if (
                    not rule_manifest_errors
                    and not judge_manifest_errors
                    and isinstance(history_key, str)
                ):
                    try:
                        expected_history_key = evaluated_version_key(
                            capability["taskIdentityKey"],
                            target_commit,
                            snapshot["rulebookManifest"]["rulebookDigest"],
                            snapshot["judgeManifest"]["judgeDigest"],
                        )
                    except (ContractError, KeyError, TypeError):
                        errors.append(
                            f"{prefix}.evaluatedSnapshot identity inputs are invalid"
                        )
                    else:
                        if history_key != expected_history_key:
                            errors.append(
                                f"{prefix}.evaluatedSnapshot evaluatedVersionKey "
                                "does not match retained task and immutable inputs"
                            )

        if not isinstance(evidence_queue, list) or not evidence_queue:
            errors.append(f"{prefix}.queueItems must be a non-empty list")
            evidence_queue = []
        try:
            evidence_queue_digest = queue_items_digest(evidence_queue)
        except ContractError as error:
            errors.append(f"{prefix}: {error}")
            evidence_queue_digest = None

        entry_pairs: set[tuple[str, str]] = set()
        for item_index, evidence_item in enumerate(evidence_queue):
            item_prefix = f"{prefix}.queueItems[{item_index}]"
            if not isinstance(evidence_item, Mapping):
                errors.append(f"{item_prefix} must be a mapping")
                continue
            errors.extend(
                f"{item_prefix}: {error}"
                for error in validate_queue_item(
                    evidence_item,
                    history_evidence_is_validated_by_bundle=True,
                )
            )
            fingerprint = evidence_item.get("fingerprint")
            if not isinstance(fingerprint, str) or not isinstance(history_key, str):
                continue
            if evidence_item.get("sliceId") != capability.get("sliceId"):
                errors.append(
                    f"{item_prefix} sliceId does not bind this Capability Slice"
                )
            if evidence_item.get("evaluatedVersionKey") != history_key:
                errors.append(
                    f"{item_prefix} evaluatedVersionKey does not bind its "
                    "historical snapshot"
                )
            pair = (fingerprint, history_key)
            if pair in entry_pairs or pair in evidence_items:
                errors.append(
                    f"{item_prefix} duplicates retained Queue history state"
                )
                continue
            entry_pairs.add(pair)
            current_item = current_items.get(fingerprint)
            expected_state = required_states.get(pair)
            if current_item is None or expected_state is None:
                errors.append(
                    f"{item_prefix} does not correspond to a non-current "
                    "initialStateHistory entry"
                )
                continue
            if not _canonical_json_equal(
                _queue_immutable_root(evidence_item),
                _queue_immutable_root(current_item),
            ):
                errors.append(
                    f"{item_prefix} rewrites the Queue Item immutable root"
                )
            for field in QUEUE_STATE_FIELDS:
                if evidence_item.get(field) != expected_state.get(field):
                    errors.append(
                        f"{item_prefix} {field} does not match initialStateHistory"
                    )
            evidence_items[pair] = evidence_item
            if target_commit is not None:
                evidence_targets[pair] = target_commit

        judge_registry: dict[str, Mapping[str, Any]] = {}
        registered_rule_ids: set[str] = set()
        if history_policy is not None and target_commit is not None:
            judge_registry, judge_registry_errors = load_judge_registry(
                repository,
                target_commit,
                history_policy,
            )
            errors.extend(f"{prefix}: {error}" for error in judge_registry_errors)
            if any(
                isinstance(item, Mapping)
                and isinstance(item.get("failureSource"), Mapping)
                and item["failureSource"].get("type") == "rule"
                ...
                errors.extend(f"{prefix}: {error}" for error in payload_errors)
                registered_rule_ids = {
                    payload["ruleId"]
                    for payload in registered_payloads.values()
                    if isinstance(payload.get("ruleId"), str)
                }

        external_execution_check_ids = {
            source["checkId"]
            for item in evidence_queue
            if isinstance(item, Mapping)
            and isinstance((source := item.get("failureSource")), Mapping)
            and source.get("type") in {"build", "test"}
            and isinstance(source.get("checkId"), str)
        }
        valid_pass_reviews: list[Mapping[str, Any]] = []
        if not isinstance(evidence_reviews, list):
            errors.append(f"{prefix}.reviewResults must be a list")
            evidence_reviews = []
        for review_index, review in enumerate(evidence_reviews):
            review_prefix = f"{prefix}.reviewResults[{review_index}]"
            if not isinstance(review, Mapping):
                errors.append(f"{review_prefix} must be a mapping")
                continue
            ...
            if isinstance(review_id, str):
                if review_id in seen_review_ids:
                    errors.append(
                        f"{review_prefix} reviewResultId is reused in this bundle"
                    )
                seen_review_ids.add(review_id)
            if isinstance(review_key, str):
                if review_key in seen_review_keys:
                    errors.append(
                        f"{review_prefix} reviewIdempotencyKey is reused in this bundle"
                    )
                seen_review_keys.add(review_key)
            review_errors = validate_review_result(
                review,
                trusted_reviewers=trusted_reviewers,
                expected_task_identity_key=capability.get("taskIdentityKey")
                if isinstance(capability.get("taskIdentityKey"), str)
                else None,
                expected_snapshot=snapshot,
                expected_queue_digest=evidence_queue_digest,
                expected_judge_registry=judge_registry,
                expected_external_execution_check_ids=external_execution_check_ids,
            )
            if review.get("reviewPurpose") != "implementation":
                review_errors.append(
                    "Queue history evidence requires implementation Review Results"
                )
            errors.extend(
                f"{review_prefix}: {error}" for error in review_errors
            )
            if not review_errors and review.get("verdict") == "PASS":
                valid_pass_reviews.append(review)

        if (
            len(evidence_reviews) != 2
            or len(valid_pass_reviews) != 2
            or len({review.get("reviewerId") for review in valid_pass_reviews}) != 2
            or len({review.get("keyId") for review in valid_pass_reviews}) != 2
        ):
            errors.append(
                f"{prefix}: Queue history evidence requires exactly two "
                "independent valid signed reviews"
            )
        for check_id in capability.get("judgeCommands", []):
            ...
            for review in valid_pass_reviews:
                matches = [
                    execution
                    for execution in review.get("commandsRun", [])
                    if isinstance(execution, Mapping)
                    and execution.get("checkId") == check_id
                    and execution.get("exitCode") == 0
                ...
                if len(matches) != 1:
                    errors.append(
                        f"{prefix}: Queue history evidence checkId {check_id} "
                        "requires one signed successful Judge execution from "
                        "each reviewer"
                    )
        errors.extend(
            f"{prefix}: {error}"
            for error in validate_review_finding_queue_traceability(
                valid_pass_reviews,
                evidence_queue,
            )
        )
        if target_commit is not None:
            errors.extend(
                f"{prefix}: {error}"
                for error in validate_non_review_queue_traceability(
                    valid_pass_reviews,
                    evidence_queue,
                    judge_registry,
                    registered_rule_ids,
                    set(capability.get("ruleIds", [])),
                    repository,
                    target_commit,
                )
            )

    for pair, state in required_states.items():
        if pair not in evidence_items:
            fingerprint, _history_key = pair
            errors.append(
                f"Queue Item {fingerprint} non-current initialStateHistory entry "
                "requires retained signed history evidence "
                f"(status {state.get('status')})"
            )

    current_target = current_snapshot.get("targetCommitSha")
    for fingerprint, current_item in current_items.items():
        history = current_item.get("initialStateHistory")
        if not isinstance(history, list) or not isinstance(current_target, str):
            continue
        ...
        for state in history:
            if not isinstance(state, Mapping):
                ...
                break
            history_key = state.get("evaluatedVersionKey")
            if history_key == current_item.get("evaluatedVersionKey"):
                history_contains_current = True
                for field in QUEUE_STATE_FIELDS:
                    if current_item.get(field) != state.get(field):
                        errors.append(
                            f"Queue Item {fingerprint} current {field} does not "
                            "match initialStateHistory"
                        )
                ordered_items.append(current_item)
                ordered_targets.append(current_target)
                continue
            pair = (fingerprint, str(history_key))
            historical_item = evidence_items.get(pair)
            historical_target = evidence_targets.get(pair)
            if historical_item is None or historical_target is None:
                missing_state = True
                break
            ordered_items.append(historical_item)
            ordered_targets.append(historical_target)
        if missing_state:
            continue
        for previous_item, next_item in zip(ordered_items, ordered_items[1:]):
            errors.extend(
                _validate_queue_state_transition(
                    previous_item,
                    next_item,
                    fingerprint,
                )
            )
        for target_index, (previous_target, next_target) in enumerate(
            zip(ordered_targets, ordered_targets[1:]),
            start=1,
        ):
            ancestry_valid = previous_target != next_target
            if ancestry_valid:
                try:
                    validate_git_ancestor(
                        repository,
            ...
            if not ancestry_valid:
                errors.append(
                    f"Queue Item {fingerprint} initialStateHistory target commits "
                    "must form a strict ancestor chain"
                )
            try:
                next_parents = commit_parents(repository, next_target)
            except EvidenceError:
                errors.append(
                    f"Queue Item {fingerprint} initialStateHistory target parent "
                    "cannot be resolved"
                )
            else:
                if len(next_parents) != 1:
                    errors.append(
                        f"Queue Item {fingerprint} initialStateHistory transition "
                        f"{target_index} must use a single-parent snapshot or a "
                        "later single-parent reconciliation"
                    )
        if (
            ordered_targets
            and not history_contains_current
            and ordered_targets[-1] != current_target
        ):
            try:
                validate_git_ancestor(
                    repository,
                ...
                errors.append(
                    f"Queue Item {fingerprint} retained bootstrap target is not "
                    "an ancestor of the current evaluated snapshot"
                )
    return errors
...
def validate_bundle(
    ...
    *,
    ...
    if not BUNDLE_FIELDS.issubset(bundle) or set(bundle) - (
        BUNDLE_FIELDS | BUNDLE_OPTIONAL_FIELDS
    ):
    ...
    external_execution_check_ids: set[str] = set()
    has_non_review_queue_sources = False
    has_rule_queue_sources = False
    if isinstance(raw_queue, list):
        for item in raw_queue:
            if not isinstance(item, Mapping):
                continue
            source = item.get("failureSource")
            if isinstance(source, Mapping) and source.get("type") == "rule":
                has_rule_queue_sources = True
            if isinstance(source, Mapping) and source.get("type") in {
                "judge",
                "build",
                "test",
            }:
                has_non_review_queue_sources = True
            if (
                isinstance(source, Mapping)
                and source.get("type") in {"build", "test"}
                and isinstance(source.get("checkId"), str)
            ):
                external_execution_check_ids.add(source["checkId"])
    try:
    ...
    registered_rule_ids: set[str] = set()
    if has_rule_queue_sources:
        registered_payloads, registered_payload_errors = (
            _load_registered_rule_payloads(
                repository,
                target_commit,
                evaluated_policy,
            )
        )
        errors.extend(registered_payload_errors)
        for rule_path, payload in registered_payloads.items():
            rule_id = payload.get("ruleId")
            if not isinstance(rule_id, str):
                continue
            if rule_id in registered_rule_ids:
                errors.append(
                    "evaluated Rule Card registry contains a duplicate ruleId: "
                    f"{rule_id}"
                )
                continue
    ...
    if (
        ...
        or has_non_review_queue_sources
    ):
        judge_registry, judge_registry_errors = load_judge_registry(
    ...
    if not isinstance(raw_queue, list):
        ...
        for index, item in enumerate(raw_queue):
            if not isinstance(item, Mapping):
                ...
                continue
            item_errors = validate_queue_item(
                item,
                history_evidence_is_validated_by_bundle=True,
            )
        ...
        errors.extend(
            validate_non_review_queue_traceability(
                valid_reviews,
                raw_queue,
                judge_registry,
                registered_rule_ids,
                set(declared_rule_ids),
                repository,
                target_commit,
            )
        )
        errors.extend(
            validate_queue_history_evidence(
                bundle.get("queueHistoryEvidence", []),
                queue_items=raw_queue,
                capability=capability,
                current_snapshot=evaluated,
                repository=repository,
                baseline_policy=baseline_policy,
                trusted_reviewers=trusted_reviewers,
                current_review_ids=set(reviews),
                current_review_keys=seen_review_keys,
            )
        )
    return errors
...
def validate_queue_history_step(
    ...
    for fingerprint, item in current_states.items():
        ...
        if previous is None:
            history_errors, initial_final_state = (
                validate_queue_initial_state_history(
                    item.get("initialStateHistory"),
                    fingerprint,
                    current_evaluated_version_key=item.get(
                        "evaluatedVersionKey"
                    )
                    if isinstance(item.get("evaluatedVersionKey"), str)
                    else None,
            ...
            errors.extend(history_errors)
            if initial_final_state is not None and any(
                item.get(field) != initial_final_state.get(field)
                for field in QUEUE_STATE_FIELDS
            ):
                errors.append(
                    f"Queue Item {fingerprint} initialStateHistory final state "
                    "must match its first persisted state"
                )
            continue
        immutable_previous = {
        ...
        if not _canonical_json_equal(immutable_previous, immutable_current):
            errors.append(f"Queue Item {fingerprint} immutable root was rewritten")
        errors.extend(
            _validate_queue_state_transition(previous, item, fingerprint)
        )
    return errors
    ...
    return {
        ...
        for fingerprint in {
            ...
            if any(
                ...
                for other in parent_states
            )
        }
    }


def _queue_activation_fingerprints(
    parent_states: Sequence[Mapping[str, Mapping[str, Any]]],
    current_state: Mapping[str, Mapping[str, Any]],
) -> set[str]:
    """Return Queue states created here instead of inherited from a parent."""

    fingerprints = set(current_state)
    for parent_state in parent_states:
        fingerprints.update(parent_state)
    return {
        fingerprint
        for fingerprint in fingerprints
        if all(
            parent_state.get(fingerprint) != current_state.get(fingerprint)
            for parent_state in parent_states
        )
    }
...
def validate_queue_merge_step(
    ...
    if not filtered_parents:
        return conflicts, []
    if any(
        not _canonical_json_equal(state, filtered_parents[0])
        for state in filtered_parents[1:]
    ):
...
def validate_queue_reconciliation(
    ...
    if _canonical_json_equal(current_item, direct_parent_item):
        return [
    ...
    if not immutable_roots or any(
        not _canonical_json_equal(root, immutable_roots[0])
        for root in immutable_roots
    ):
        errors.append(f"Queue Item {fingerprint} merge parents rewrote its immutable root")
        return errors
    if not _canonical_json_equal(current_root, immutable_roots[0]):
...
def _collect_approval_state(
    ...
    *,
    ...
    for artifact_path in artifact_paths:
        ...
        review_groups: list[Any] = [bundle.get("reviewResults")]
        raw_history_evidence = bundle.get("queueHistoryEvidence")
        if isinstance(raw_history_evidence, list):
            review_groups.extend(
                evidence_entry.get("reviewResults")
                for evidence_entry in raw_history_evidence
                if isinstance(evidence_entry, Mapping)
            )
        for raw_reviews in review_groups:
            if not isinstance(raw_reviews, list):
                continue
        ...
        for item in bundle.get("queueItems", []):
            ...
            if not _canonical_json_equal(previous, item):
                errors.append(
...
def _collect_queue_target_commits(
    repository: Path,
    commit_sha: str,
) -> tuple[list[str], dict[str, str]]:
    """Map each signed Queue state to the evaluated tree it represents."""
    ...
    try:
        ...
        return [str(error)], target_commits
    for artifact_path in (path for path in tracked_paths if path.endswith(".json")):
        try:
            bundle = load_bundle_git(repository, commit_sha, artifact_path)
        except ContractError:
            continue
        evaluated_snapshot = bundle.get("evaluatedSnapshot")
        if not isinstance(evaluated_snapshot, Mapping):
            continue
        target_commit = evaluated_snapshot.get("targetCommitSha")
        if not isinstance(target_commit, str):
            continue
        queue_items = bundle.get("queueItems")
        if not isinstance(queue_items, list):
            continue
        for item in queue_items:
            if not isinstance(item, Mapping) or not isinstance(
                ...
                continue
            fingerprint = item["fingerprint"]
            previous_target = target_commits.setdefault(fingerprint, target_commit)
            if previous_target != target_commit:
                errors.append(
                    f"Queue Item {fingerprint} binds conflicting evaluated target commits"
                )
    return errors, target_commits
...
def _codeowners_pattern_matches(pattern: str, path: str) -> bool:
    ...
    terminal_directory_globstar = directory_pattern and (
        normalized == "**" or normalized.endswith("/**")
    )
    index = 0
    while index < len(normalized):
        ...
        if character == "*":
            if index + 1 < len(normalized) and normalized[index + 1] == "*":
                index += 2
                if terminal_directory_globstar and index == len(normalized):
                    expression.append(r".+")
                    continue
    ...
    if directory_pattern and not terminal_directory_globstar:
        expression.append(r"/.*")
    elif not any(character in last_segment for character in "*?"):
        expression.append(r"(?:/.*)?")
    expression.append("$")
    return re.match("".join(expression), path) is not None


def codeowners_for_path(
    rules: Sequence[tuple[str, tuple[str, ...]]], path: str
) -> tuple[str, ...]:
    owners: tuple[str, ...] = ()
...
def validate_governance_coverage(
    ...
    for path in protected_paths:
        owners = codeowners_for_path(rules, path)
...
def validate_repository_artifacts(
    ...
    *,
    ...
    historical_queue_version_keys: dict[str, set[str]] = {}
    ...
    for history_commit in dict.fromkeys([*artifact_history, commit_sha]):
        ...
        for fingerprint, item in queue_states.items():
            seen_version_keys = historical_queue_version_keys.setdefault(
                fingerprint, set()
            )
            parent_items = [
                state[fingerprint]
            ...
            if not parent_items:
                initial_history = item.get("initialStateHistory")
                if isinstance(initial_history, list):
                    seen_version_keys.update(
                        state["evaluatedVersionKey"]
                        for state in initial_history
                        if isinstance(state, Mapping)
                        and isinstance(state.get("evaluatedVersionKey"), str)
                    )
            current_version_key = item.get("evaluatedVersionKey")
            state_changed_from_every_parent = bool(parent_items) and all(
                not _canonical_json_equal(item, parent_item)
                for parent_item in parent_items
            )
            if (
                state_changed_from_every_parent
                and isinstance(current_version_key, str)
                and current_version_key in seen_version_keys
            ):
                errors.append(
                    f"historical artifact commit {history_commit}: Queue Item "
                    f"{fingerprint} reuses a historical evaluatedVersionKey"
                )
            if isinstance(current_version_key, str):
                seen_version_keys.add(current_version_key)
        if history_commit != anchor_commit:
            activated_queue_fingerprints = _queue_activation_fingerprints(
                parent_queue_states,
                queue_states,
            )
            if activated_queue_fingerprints:
                if len(activation_parents) != 1:
                    errors.append(
                        f"Queue state activation commit {history_commit} must have exactly one parent"
                    )
                changed_paths = changed_paths_for_commit(
                    canonical_repository, history_commit
                )
                ...
                if any(
                    ...
                    errors.append(
                        f"Queue state activation commit {history_commit} contains changes outside canonical JSON envelopes"
                    )
                target_errors, queue_target_commits = (
                    _collect_queue_target_commits(
                        canonical_repository,
                        history_commit,
                    )
                )
                errors.extend(
                    f"Queue state activation commit {history_commit}: {error}"
                    for error in target_errors
                )
                if len(activation_parents) == 1:
                    direct_parent = activation_parents[0]
                    for fingerprint in sorted(activated_queue_fingerprints):
                        if queue_target_commits.get(fingerprint) != direct_parent:
                            errors.append(
                                f"Queue state activation commit {history_commit} "
                                f"must bind its single parent tree {direct_parent} "
                                f"for Queue Item {fingerprint}"
                            )
            if len(parent_queue_states) == 1:
                errors.extend(
                    f"historical artifact commit {history_commit}: {error}"
                    for error in validate_queue_history_step(
                        parent_queue_states[0], queue_states
                    )
                )
            elif len(parent_queue_states) > 1:
                first_parent_state = parent_queue_states[0]
                if all(
                    _canonical_json_equal(state, first_parent_state)
                    for state in parent_queue_states[1:]
                ):
                    errors.extend(
                        f"historical artifact commit {history_commit}: {error}"
                        for error in validate_queue_history_step(
                            first_parent_state, queue_states
                        )
                    )
                else:
