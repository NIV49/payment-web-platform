...
public final class DefaultScopeMatcher {
    ...
    public boolean matches(AuthorizationSubject subject, DimensionScope scope, ResourceContext resource) {
        return switch (scope.mode()) {
            ...
            case ASSIGNED -> matchesRelationship(subject, scope, resource);
            case SPECIFIED -> {
                String resourceValue = resource.valueOf(scope.dimension());
                yield resourceValue != null && scope.targets().contains(resourceValue);
            }
            case RELATION_CURRENT, RELATION_AT_EVENT -> matchesRelationship(subject, scope, resource);
        };
    }

    private boolean matchesRelationship(AuthorizationSubject subject,
                                        DimensionScope scope,
                                        ResourceContext resource) {
        try {
            return relationshipScope.matches(subject, scope, resource);
