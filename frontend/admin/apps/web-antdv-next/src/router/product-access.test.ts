...
import { parseAccountDomain } from '#/deployment';
import * as agentPolicy from '#/deployments/agent';
import * as merchantPolicy from '#/deployments/merchant';
...
function routePolicy(deployment: typeof agentPolicy | typeof merchantPolicy) {
  return {
    accountDomain: deployment.DEPLOYMENT_ACCOUNT_DOMAIN,
    menuPageComponents: deployment.DEPLOYMENT_MENU_PAGE_COMPONENTS,
    routeNames: deployment.DEPLOYMENT_ROUTE_NAMES,
    routePaths: deployment.DEPLOYMENT_ROUTE_PATHS,
  };
}
...
describe('product access policy', () => {
  ...
  it.each([
    ['agent', routePolicy(agentPolicy), 'AgentDashboard', 'AgentWorkspace'],
    [
      'merchant',
      routePolicy(merchantPolicy),
      'MerchantDashboard',
      'MerchantWorkspace',
    ],
  ] as const)(
